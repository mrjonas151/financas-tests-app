...
describe('Ao calcular o saldo do usuário...', () => {
    ...
    test('Deve adicionar valores de entradas', () => {
    ...
    test('Deve subtrair valores de saídas', () => {
        return request(app).get(ROUTE_TRANSACTION)
    ...
    test('Deve considerar transferências', () => {
        return request(app).get(ROUTE_TRANSACTION)
            .send({ description: '1', date: new Date(), ammount: 250, acc_ori_id: 10100, acc_dest_id: 10101, status: true })
            .set('authorization', `bearer ${TOKEN}`)
            .then(() => {
                return request(app).get(MAIN_ROUTE)
                    .set('authorization', `bearer ${TOKEN}`)
